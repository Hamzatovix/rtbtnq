  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-inkSoft/60">
        ...
        <span>/</span>
        <span className="text-inkSoft">Создать</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-title-1 font-light mb-2">Создать товар</h1>
          <p className="text-sm text-inkSoft/60">Заполните информацию о новом товаре</p>
        </div>
        <Link href="/backoffice/products" className="text-sageTint hover:underline inline-flex items-center gap-2 transition-colors self-start sm:self-auto">
      ...
      <form onSubmit={onSubmit} className="space-y-8">
        {/* Основная информация */}
        <div className="bg-white rounded-2xl border border-mistGray/20 shadow-sm p-6 md:p-8 space-y-6">
          <div className="flex items-center gap-2 pb-4 border-b border-mistGray/20">
            <h2 className="text-lg font-medium text-inkSoft">Основная информация</h2>
            <span className="text-xs text-inkSoft/40 bg-roseBeige/30 px-2 py-1 rounded-full">Обязательно</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input 
                ...
                required 
              />
              <p className="mt-1 text-xs text-inkSoft/60">Название товара для витрины</p>
            </div>
            <div>
              ...
              <Input 
                ...
                required 
              />
              <p className="mt-1 text-xs text-inkSoft/60">URL-адрес товара (генерируется автоматически)</p>
            </div>
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="w-full min-h-32 rounded-2xl border border-mistGray/30 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-sageTint focus:border-sageTint resize-none"
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                className="w-full min-h-24 rounded-2xl border border-mistGray/30 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-sageTint focus:border-sageTint resize-none"
            ...
            <div>
              ...
              <select 
                className="w-full h-11 rounded-2xl border border-mistGray/30 bg-white px-4 text-sm outline-none focus:ring-2 focus:ring-sageTint focus:border-sageTint" 
                value={categoryId} 
            ...
            <div className="flex items-center gap-3 pt-6">
              <input 
                ...
                className="w-4 h-4 rounded border-mistGray/30 text-sageTint focus:ring-sageTint focus:ring-2"
              />
              <Label htmlFor="pub" className="cursor-pointer">Опубликовать товар сразу</Label>
            </div>
          </div>
        </div>
        ...
        {/* Варианты товара */}
        <div className="bg-white rounded-2xl border border-mistGray/20 shadow-sm p-6 md:p-8 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-mistGray/20">
            <div>
              <h2 className="text-lg font-medium text-inkSoft">Варианты товара</h2>
              <p className="text-sm text-inkSoft/60 mt-1">Цвета, размеры и другие характеристики</p>
            </div>
            <Button 
              type="button" 
              variant="outline" 
          ...
          {variants.map((v, idx)=> {
            ...
            <div key={idx} className={`p-6 border-2 rounded-2xl space-y-4 transition-all ${
              isComplete 
                ...
                : 'border-mistGray/30 bg-roseBeige/20'
            }`}>
              <div className="flex items-center justify-between pb-3 border-b border-mistGray/20">
                <div className="flex items-center gap-3">
                  <h3 className="font-medium text-inkSoft">Вариант {idx + 1}</h3>
                  {isComplete && (
                    ...
                    {colors.map((c)=>{
                      ...
                      return (
                        <button
                          ...
                          className={`relative w-10 h-10 rounded-full border-2 transition-all ${selected ? 'ring-2 ring-sageTint border-sageTint scale-110' : 'border-mistGray/40 hover:border-mistGray/60 hover:scale-105'}`}
                ...
                <div className="flex items-end">
                  <label className="inline-flex items-center gap-2 cursor-pointer">
                    <input 
                      ...
                      className="w-4 h-4 rounded border-mistGray/30 text-sageTint focus:ring-sageTint focus:ring-2"
                    />
                    <span className="text-sm">Вариант по умолчанию</span>
                  </label>
                </div>
              </div>
              <div>
          ...
          {variants.length > 0 && (
            <div className="pt-4 border-t border-mistGray/20">
              <Button 
                type="button" 
                variant="outline" 
                onClick={addVariant}
        ...
        {/* Кнопки действий */}
        <div className="bg-white rounded-2xl border border-mistGray/20 shadow-sm p-6">
          {error && (
