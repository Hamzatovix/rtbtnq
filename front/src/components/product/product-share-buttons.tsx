...
export function ProductShareButtons({
  ...
  const createShareImage = async (): Promise<File> => {
    const telegramImage = await createTelegramImage()
    return new File([telegramImage], telegramImage.name.replace(/_telegram\.jpg$/i, '_share.jpg'), {
      type: telegramImage.type || 'image/jpeg',
      lastModified: Date.now(),
    })
  }
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs font-mono uppercase tracking-[0.15em] text-fintage-graphite dark:text-fintage-offwhite">
          Поделиться
        </span>
