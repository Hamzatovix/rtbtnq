#!/bin/bash
# Проверка и применение SSL настроек

set -e

echo "🔍 Проверка SSL настроек"
echo "========================"
echo ""

# 1. Проверка файлов сертификатов
echo "1️⃣  Проверка файлов сертификатов..."
if [ -f "/root/Shtraf/nginx/ssl/rosebotanique/certificate.crt" ] && [ -f "/root/Shtraf/nginx/ssl/rosebotanique/certificate.key" ]; then
    echo "✅ Файлы сертификатов найдены"
    ls -lh /root/Shtraf/nginx/ssl/rosebotanique/
else
    echo "❌ Файлы сертификатов не найдены!"
    exit 1
fi
echo ""

# 2. Проверка docker-compose.yml
echo "2️⃣  Проверка docker-compose.yml..."
if grep -q "rosebotanique/certificate.crt" /root/Shtraf/docker-compose.yml; then
    echo "✅ Volumes для сертификатов добавлены в docker-compose.yml"
else
    echo "❌ Volumes для сертификатов НЕ найдены в docker-compose.yml!"
    echo "   Нужно добавить:"
    echo "   - ./nginx/ssl/rosebotanique/certificate.crt:/etc/ssl/certs/rosebotanique.store.crt:ro"
    echo "   - ./nginx/ssl/rosebotanique/certificate.key:/etc/ssl/private/rosebotanique.store.key:ro"
    exit 1
fi
echo ""

# 3. Проверка nginx.conf - SSL директивы
echo "3️⃣  Проверка SSL директив в nginx.conf..."
if grep -q "^ssl_certificate.*rosebotanique.store.crt" /root/Shtraf/nginx/nginx.conf; then
    echo "✅ SSL директивы раскомментированы"
else
    echo "❌ SSL директивы всё ещё закомментированы!"
    echo "   Нужно убрать # перед ssl_certificate и ssl_certificate_key"
    exit 1
fi
echo ""

# 4. Проверка HTTP блока - редирект
echo "4️⃣  Проверка HTTP блока..."
if grep -A 5 "Rosebotanique.store HTTP" /root/Shtraf/nginx/nginx.conf | grep -q "return 301 https"; then
    echo "✅ HTTP блок настроен на редирект на HTTPS"
else
    echo "⚠️  HTTP блок может быть не настроен на редирект"
    echo "   Проверь, что есть: return 301 https://\$host\$request_uri;"
fi
echo ""

# 5. Перезапуск контейнера
echo "5️⃣  Перезапуск контейнера Nginx..."
docker restart shtrafnett_nginx
sleep 3
echo "✅ Контейнер перезапущен"
echo ""

# 6. Проверка конфигурации Nginx
echo "6️⃣  Проверка конфигурации Nginx..."
if docker exec shtrafnett_nginx nginx -t 2>&1 | grep -q "successful"; then
    echo "✅ Конфигурация Nginx правильная"
    docker exec shtrafnett_nginx nginx -s reload
    echo "✅ Nginx перезагружен"
else
    echo "❌ Ошибка в конфигурации Nginx!"
    docker exec shtrafnett_nginx nginx -t
    exit 1
fi
echo ""

# 7. Проверка работы
echo "7️⃣  Проверка работы..."
echo "Проверка редиректа HTTP → HTTPS:"
HTTP_REDIRECT=$(curl -s -o /dev/null -w "%{http_code}" http://rosebotanique.store 2>/dev/null || echo "000")
if [ "$HTTP_REDIRECT" = "301" ] || [ "$HTTP_REDIRECT" = "000" ]; then
    echo "   ✅ HTTP редиректит на HTTPS (код: $HTTP_REDIRECT)"
else
    echo "   ⚠️  HTTP возвращает код: $HTTP_REDIRECT (ожидался 301)"
fi

echo ""
echo "Проверка HTTPS:"
HTTPS_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -k https://rosebotanique.store 2>/dev/null || echo "000")
if [ "$HTTPS_STATUS" = "200" ]; then
    echo "   ✅ HTTPS работает (код: $HTTPS_STATUS)"
else
    echo "   ⚠️  HTTPS возвращает код: $HTTPS_STATUS (ожидался 200)"
fi
echo ""

echo "🎉 Настройка SSL завершена!"
echo ""
echo "📋 Проверь в браузере:"
echo "   - http://rosebotanique.store → должен редиректить на https://"
echo "   - https://rosebotanique.store → должен открыться с зелёным замочком 🔒"

